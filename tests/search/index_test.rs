...
fn reset_clears_existing_index() {
    ...
    // Reset
    let opts2 = IndexOptions { reset: true, path: src.path().to_path_buf(), index_dir: idx.path().to_path_buf(), progress: None, embed_provider: ecotokens::config::settings::EmbedProvider::None };
