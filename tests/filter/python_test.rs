...
";
    let out = filter_python("pytest", input);
    assert!(out.contains("FAILURES"), "FAILURES section header should be kept");
    assert!(out.contains("def test_fail"), "traceback body should be kept");
    assert!(out.contains("AssertionError"), "error location should be kept");
}
...
";
    ...
    assert!(out.contains("FAILURES"), "uv run pytest should route through pytest filter");
...
fn uv_pip_install_is_routed_correctly() {
    let mut input = String::new();
    for i in 0..30 {
        input.push_str(&format!("Collecting dep{i}\n  Downloading dep{i}-1.0.whl\n"));
    }
    input.push_str("Successfully installed dep1 dep2\n");
    let out = filter_python("uv pip install .", &input);
    assert!(out.contains("Successfully installed"), "uv pip should route through pip filter");
    assert!(!out.contains("Collecting dep10"), "download noise should be omitted");
}
...
";
    ...
    assert!(out.contains("ModuleNotFoundError"), "import error detail should be kept");
}
...
fn pip_error_is_kept() {
    ...
    assert!(!out.contains("Downloading dep10"), "download noise should be omitted");
}
...
fn ruff_many_errors_keeps_all_structured_lines() {
    let mut input = String::new();
    for i in 0..120 {
        input.push_str(&format!("src/file{i}.py:{i}:1: F401 `os` imported but unused\n"));
    }
    input.push_str("Found 120 errors.\n");
    let out = filter_python("ruff check .", &input);
    // All diagnostic lines should be kept (no head+tail truncation in the middle)
    assert!(out.contains("src/file60.py"), "middle errors should not be truncated by head+tail");
    assert!(out.contains("Found 120 errors."), "summary line should be kept");
}
